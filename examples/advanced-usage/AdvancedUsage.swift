        ...
        do {
            ...
            if let txData = try? JSONSerialization.jsonObject(with: txResponse) as? [String: Any],
               let result = txData["result"] as? [String: Any] {
                print("   ✅ Transaction Hash: \(result["transaction"] as? [String: Any]?["hash"] as? String ?? "N/A")")
                print("   ✅ Status: \(result["status"] as? [String: Any]?["SuccessValue"] != nil ? "Success" : "Failed")")
            }
            ...
            if let chunkData = try? JSONSerialization.jsonObject(with: chunkResponse) as? [String: Any],
               let result = chunkData["result"] as? [String: Any] {
                print("   ✅ Chunk Hash: \(result["header"] as? [String: Any]?["chunk_hash"] as? String ?? "N/A")")
                print("   ✅ Shard ID: \(result["header"] as? [String: Any]?["shard_id"] as? Int ?? 0)")
            }
